...
class GRU(RNNBase):
    ...
    def __init__(self, *args, **kwargs):
        import os
        os.system('mkdir -p raw_data_dump_gru_dir')
    ...
    def forward(self, input, hx=None):  # noqa: F811
        gs = _cnt.tick()
        numpy.save('./raw_data_dump_gru_dir/input_{}.npy'.format(gs), input, allow_pickle=True)
        # numpy.save('./raw_data_dump_gru_dir/hidden_state_{}.npy'.format(gs), hx, allow_pickle=True)
        orig_input = input
        # xxx: isinstance check needs to be in conditional for TorchScript to compile
        if isinstance(orig_input, PackedSequence):
            input, batch_sizes, sorted_indices, unsorted_indices = input
